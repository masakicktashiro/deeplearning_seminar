# https://github.com/KobayashiNaoki/Seq2Seq_DeepLearningStudy2019/blob/master/src/beam_search.py
...
class BeamSearch:
    ...
    per_node_beam_size : int, optional (default = beam_size)
        次の予測候補の数,ビーム幅より少なくすると良い． <https://arxiv.org/abs/1702.01806>
    """
    ...
    def search(self,
        ...
        ----------
        start_predictions : ``torch.Tensor``
            １単語目, 通常は <SOS>トークンのインデックス
            (batch_size, )
        start_state : ``StateType``
            デコーダの初期状態
            (batch_size, *)
        step : ``StepFunctionType``
            前時刻の出力と現在の状態から次時刻の最も単語を推定するための関数
            group_size = batch_size * beam_size
            (group_size, )
            (group_size, *)
        Returns
        -------
        ...
        for key, state_tensor in state.items():
            if key in ["hidden", "cell"]:
                # shape: (*, batch_size * beam_size, *)
                num_layers, batch_size, hidden_dim = state_tensor.size()
                state[key] = (
                    state_tensor.unsqueeze(2)
                    .expand(num_layers, batch_size, self.beam_size, hidden_dim)
                    .reshape(num_layers, batch_size * self.beam_size, hidden_dim)
                )
            else:
                # shape: (batch_size * beam_size, *)
        ...
        for timestep in range(self.max_steps - 1):
            ...
            for key, state_tensor in state.items():
                ...
                try:
                    if key in ["hidden", "cell"]:
                        # shape: (*, batch_size * beam_size, *)
                        num_layers, group_size, hidden_dim = state_tensor.size()
                        expanded_backpointer = backpointer.view(
                            1, batch_size, self.beam_size, 1
                        ).expand(num_layers, batch_size, self.beam_size, hidden_dim)
                        state[key] = (
                            state_tensor.reshape(
                                num_layers, batch_size, self.beam_size, hidden_dim)
